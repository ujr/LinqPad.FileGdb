...
public class GeometryTest
{
	...
	public void CanSignedAndHeronArea()
	{
		...
		Assert.Equal(5.0, ShapeEngine.HeronArea(a, b, c), precision);
		Assert.Equal(5.0, ShapeEngine.HeronArea(c, b, a), precision);

		Assert.Equal(-5.0, ShapeEngine.SignedArea(a, b, c), precision);
		Assert.Equal(5.0, ShapeEngine.SignedArea(c, b, a), precision);
	}
	...
	public void CanCircumcircle()
	{
		...
		Assert.Equal(Math.Sqrt(5), ShapeEngine.CircumcircleRadius(a, b, c), precision);

		var r = ShapeEngine.Circumcircle(a, b, c, out var center);
		Assert.Equal(Math.Sqrt(5), r, precision);
		Assert.Equal(3.0, center.X, precision);
		Assert.Equal(2.0, center.Y, precision);

		r = ShapeEngine.CircumcircleOld(a, b, c, out center);
		Assert.Equal(Math.Sqrt(5), r, precision);
		Assert.Equal(3.0, center.X, precision);
		Assert.Equal(2.0, center.Y, precision);

		// Collinear points (circumcircle does not exist):
		r = ShapeEngine.Circumcircle(new XY (1,1), new XY (2,2), new XY (4,4), out center);
		Assert.True(double.IsInfinity(r) && center.IsEmpty);

		// All three points the same (circumcircle does not exist):
		r = ShapeEngine.Circumcircle(a, a, a, out center);
		Assert.True(double.IsInfinity(r) && center.IsEmpty);
	...
	public void CanCentralAngle()
	{
		...
		Assert.Equal(0.6435, ShapeEngine.CentralAngle(a, o, b), precision);
		Assert.Equal(5.6397, ShapeEngine.CentralAngle(a, o, b, true), precision);
		Assert.Equal(5.6397, ShapeEngine.CentralAngle(b, o, a), precision);
		Assert.Equal(0.6435, ShapeEngine.CentralAngle(b, o, a, true), precision);


		Assert.Equal(0.0, ShapeEngine.CentralAngle(a, o, a), 9);
		Assert.Equal(2 * Math.PI, ShapeEngine.CentralAngle(a, o, a, true), 9);
